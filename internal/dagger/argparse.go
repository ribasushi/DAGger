...
func (cfg *config) initArgvParser() {
	...
	o.FlagLong(&cfg.hashFunc, "hash", 0, "Hash function to use, one of: "+text.AvailableMapKeys(dgrblock.AvailableHashers),
		"algname",
	)
...
func (dgr *Dagger) setupCarWriting() (argErrs []string) {
	...
	if stream.IsTTY(carSelectedOut) {
...
func (dgr *Dagger) setupEncoding() (nodeEnc dgrencoder.NodeEncoder, argErrs []string) {
	...
	if _, exists := dgrblock.AvailableHashers[cfg.hashFunc]; !exists {
		...
		argErrs = append(argErrs, fmt.Sprintf(
			...
			cfg.hashFunc,
			text.AvailableMapKeys(dgrblock.AvailableHashers),
		))
	} else {
	...
	if dgr.cfg.CidMultibase == "base32" {
		b32Encoder = base32.NewEncoding("abcdefghijklmnopqrstuvwxyz234567").WithPadding(base32.NoPadding)
	} else if dgr.cfg.CidMultibase != "base36" {
		argErrs = append(argErrs, fmt.Sprintf("unsupported cid multibase '%s'", dgr.cfg.CidMultibase))
		return
	}

	dgr.formattedCid = func(h *dgrblock.Header) (cs string) {
	...
	if !cfg.optSet.IsSet("chunkers") {
		...
		if strings.HasPrefix(ipfsOpts.Chunker, "size") {
			...
			if rabinopts[0] == "rabin" {
				...
				if bits != "" {
					cfg.requestedChunkers = fmt.Sprintf("rabin_polynomial=17437180132763653_window-size=16_state-target=0_state-mask-bits=%s_min-size=%s_max-size=%s",
						bits,
						min,
						max,
					)
