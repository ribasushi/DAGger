...
func (e *encoder) NewLeaf(ds dgrblock.DataSource) *dgrblock.Header {
	...
	dataLen := encoding.VarintSlice(uint64(ds.Size))

	blockData := zcpstring.NewWithSegmentCap(9)
	blockData.AddByte(pbHdrF1LD)
	blockData.AddSlice(encoding.VarintSlice(uint64(3 + 2*len(dataLen) + ds.Size + 1)))
...
func (e *encoder) NewLink(origin dgrencoder.NodeOrigin, blocks []*dgrblock.Header) *dgrblock.Header {
	...
	for i := range blocks {
		...
		cidLenVI := encoding.VarintSlice(uint64(len(cid)))
		var dagSizeVI []byte
		var frameLen uint64
		...
		if e.NonstandardLeanLinks {
			...
			dagSizeVI = encoding.VarintSlice(blocks[i].SizeCumulativeDag())
			frameLen = uint64(1 + len(cidLenVI) + len(cid) + 3 + len(dagSizeVI))
		}

		linkSection.AddByte(pbHdrF2LD)
		linkSection.AddSlice(encoding.VarintSlice(frameLen))
		...
		if !e.NonstandardLeanLinks {
			...
			seekOffsets.AddSlice(encoding.VarintSlice(blocks[i].SizeCumulativePayload()))
		}
	...
	payloadSizeVI := encoding.VarintSlice(totalPayload)
	...
	linkBlock.AddSlice(encoding.VarintSlice(uint64(3 + len(payloadSizeVI) + seekOffsets.Size())))
...
// 	3: 0
// }
var nulPb = zcpstring.WrapSlice([]byte("\x0a\x04\x08\x02\x18\x00"))
